...
public class NeighbouringQueryMessageHandler extends
   ...
   */
  @Override
  public void execute(Session session, QueryConfig qconf, NeighboringSegmentQuery message,
    ...
    Optional<MediaSegmentDescriptor> segmentOption = this.mediaSegmentReader
        .lookUpSegment(segmentId);
    ...
    final List<MediaSegmentDescriptor> segments = this.mediaSegmentReader
        .lookUpSegmentsByNumberRange(segment.getObjectId(),
            segment.getSequenceNumber() - message.getCount(),
            segment.getSequenceNumber() + message.getCount());
    ...
    this.loadAndWriteSegmentMetadata(session, uuid,
        segments.stream().map(MediaSegmentDescriptor::getSegmentId).collect(Collectors.toList()),
